...
def get_CRC16_check_sum(pch_message, wCRC):
    pch_message = bytearray.fromhex(pch_message)
    if pch_message is None:
        return 0xFFFF
    for i in range(len(pch_message)):
        wCRC = (wCRC >> 8) ^ wCRC_table[(wCRC ^ pch_message[i]) & 0x00ff]
    return wCRC
...
def ToBytes(i, f):
    bs = struct.pack(i,f)
    if i == '=H':
        return format(bs[0], '#04x')[2:] + format(bs[1], '#04x')[2:]
    elif i == '=B':
        return format(bs[0], '#04x')[2:]
    else:
        return format(bs[0], '#04x')[2:] + format(bs[1], '#04x')[2:] + format(bs[2], '#04x')[2:] + format(bs[3], '#04x')[2:]




def get_Bytes(f, is_datalen_or_fps):
    if not is_datalen_or_fps:
        if type(f) == float:
            i = '=f'
            B = ToBytes(i, f)
            pch_Message = B
            ...
            bb = ''
            BB = np.fromstring(f, dtype=np.uint8)
            for i in BB:
                bb = bb + format(i, '#04x')[2:]
            pch_Message = bb
    elif is_datalen_or_fps == 2:
        i = '=H'
        B = ToBytes(i, f)
        pch_Message = B
    else:
...
def Serial_communication(yaw, pitch, fps, is_autoaim):
    ...
    print('********************************')
    print(struct.pack("=cBffHi", f1, f2, f3, f4, f5, wCRC))
    data =struct.pack("=cBffHi", f1, f2, f3, f4, f5, wCRC)
...
Serial_communication(-0.395,0.2,20,1)
